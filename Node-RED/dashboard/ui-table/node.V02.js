                    ...
                    initController: function ($scope, events) {
                        ...
                        var createTable = function(basediv, tabledata, columndata, outputs, ui_control) {
                            if (!ui_control || !ui_control.tabulator) {
                                ...
                                // fill "live" defaults if there are any "gaps" in ui_control
                                if (!ui_control.tabulator.data) opts.data = tabledata;
                                if (!ui_control.tabulator.columns) opts.columns = columndata;
                                if (!ui_control.autoColumns) autoColumns = columndata.length == 0;
                                if (!ui_control.tabulator.height) opts.height= tabledata.length * y + 26;
                            }
                        ...
                        $scope.$watch('msg', function (msg) {
                            ...
                            if (msg && msg.hasOwnProperty("ui_control")) {
                                ...
                                var addValueOrFunction = function (config,param,value) {
                                    ...
                                    if ((typeof value === "string") && value.toLowerCase().includes('function') && value.includes('{') && value.includes('}')) {
                                ...
                                if (msg.ui_control.hasOwnProperty("tabulator")) {
                                    if (!$scope.config.ui_control.tabulator){
                                        $scope.config.ui_control.tabulator={
                                            layout: 'fitColumns',
                                            columns: $scope.config.columns,
                                            autoColumns: ($scope.config.columns == 0),
                                            movableColumns: true,
                                        };
                                    }
                                    addObject($scope.config.ui_control.tabulator,msg.ui_control.tabulator);
                                }
                            }
